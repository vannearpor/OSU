// Author: Vannear Por
// Date: August 17, 2018
// Description: This program is a background daemon which listens for ciphertext files to decrypt.
...
int char_number(char character) {
    switch (character) {
        ...
        case ' ': return 26;
        default:
            fprintf(stderr, "Error. Ciphertext input into otp_dec_d contains invalid characters.\n");
            exit(1);
    }
}
...
int mod(int valOne, int valTwo) {
    return (valOne + valTwo) % valTwo;
}

void decrypt(int recv_length) {
    int i;
    
    for (i = 0; i < recv_length; i++) {
        ciphertext_value[i] = char_number(cipherText[i]);     //Gives number value for plaintext data
        keytext_value[i] = char_number(keyText[i]);         //Gives number value to the key data
        plaintext_value[i] = ciphertext_value[i] - keytext_value[i];       //Cipher number value is sum of plaintext and key
        
        plaintext_value[i] = ((plaintext_value[i] + 27) % 27);     //In case the value is over 27, corrects the value
        
        if (plaintext_value[i] == 26) {
            cipherText[i] = ' ';
        }
        
        //Shift value over by 1 to give the correct letter.
        else {
            cipherText[i] = plaintext_value[i] + 'A';
        }
    }
}
...
int main (int argc, char* argv[]) {
    ...
    if (argc < 2) {
        fprintf(stderr, "Syntax Error. To run otp_dec_d, please enter: otp_dec_d listening_port\n");
        exit(1);
    }
    ...
    char send_message[] = "toDec";
    char recv_message[] = "fromDec";
    char reject[] = "CANNOT CONNECT HERE!";
    ...
    while(1) {
        ...
        if (pid == -1) {
            ...
            recv_confirm = recv(clientConnectFD, cipherText, sizeof(cipherText), 0);
            cipherText[recv_confirm] = '\0';  //null terminator at end
            
            if (strcmp(cipherText, send_message) == 0) {
                sprintf(cipherText, "%s", recv_message);
                send(clientConnectFD, cipherText, strlen(cipherText), 0);
            }
            
            else {
                sprintf(cipherText, "%s", reject);
                send(clientConnectFD, cipherText, strlen(cipherText), 0);
                exit(1);
            }
            ...
            while ((recv_length = recv(clientConnectFD, cipherText, TEXTSPACE, 0)) > 0) {
                if (recv_length == -1) {
                    perror("Recv Ciphertext Error.");
                    exit(1);
                }
                
                cipherText[recv_length] = '\0';
                ...
                send_plaintext = send(clientConnectFD, cipherText, recv_length, 0);
                if (send_plaintext == -1) {
                    perror("Error sending plaintext data back.");
                    exit(1);
                }
            }
