...
void adjustHeap(DynamicArray* heap, int last, int position, compareFunction compare)
{
    ...
    if (right < last)
    {
        ...
        if (compare(dyGet(heap, minimum), dyGet(heap, position)) == -1)
        {
            dySwap (heap, position, minimum);
            adjustHeap (heap, last, minimum, compare);
        }
    }
...
void dyHeapAdd(DynamicArray* heap, TYPE value, compareFunction compare)
{
    ...
    void* pValue = heap->data[parentIdx];
    
    while ((idx > 0) && (compare(value, pValue)) == -1)
    {
        dySwap (heap, idx, parentIdx);
        idx = parentIdx;
        parentIdx = (idx - 1)/2;
        pValue = heap->data[parentIdx];
    }
}
